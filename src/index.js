...
import { rightContainer, populateMain } from "./mainContents.js";
...
let newProjectButton = document.querySelector(".newProjectModal button");
let projectName = document.querySelector(".newProjectModal input");
...
itemSubmit.addEventListener("click", (e) => {
  ...
  let currentProject = document.querySelector(".topBar h2").textContent;
  data["cards"].push({
    name: tempData.querySelector(".titleInput").value,
    description: tempData.querySelector(".descriptionInput").value,
    dueDate: tempData.querySelector(".dueDateInput").value,
    priority: tempData.querySelector(".priorityInput").value,
    completed: "no",
  });
...
sidebarCards.addEventListener("click", (e) => {
  if (
    e.target.parentNode.className == "sideCard" ||
    e.target.parentNode.classList.contains("sideCard")
  ) {
    let temp = e.target.parentNode.querySelector("h2").textContent;
...
sidebarBtn.addEventListener("click", (e) => {
  if (e.target.className == "complete") {
    let tempStorage = JSON.parse(
      storage[e.target.parentNode.parentNode.querySelector("h2").textContent],
    );
    ...
    if (e.target.parentNode.parentNode.classList.contains("completed")) {
      ...
      storage[tempStorage.name] = JSON.stringify(tempStorage);
      data = tempStorage;
      populateMain(data);
    } else {
      e.target.parentNode.parentNode.className = "sideCard completed";
    ...
    storage.removeItem(
      e.target.parentNode.parentNode.querySelector("h2").textContent,
    );
    cards(storage);
    if (storage.length > 0) {
...
function expandCards(data) {
  ...
  document.querySelector(".infoTitle").textContent = `Name: ${data.name}`;
...
mainCardButtons.addEventListener("click", (e) => {
  if (e.target.className == "completeBtn") {
    ...
    for (let i in data["cards"]) {
      if (
        data["cards"][i]["name"] == e.target.querySelector("h3").textContent
      ) {
